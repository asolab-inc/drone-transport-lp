...
const title = p.locator('.sec-head__title');
await title.scrollIntoViewIfNeeded();
await p.waitForTimeout(200);
await title.screenshot({ path: join(OUT, 'crop-title.png') });

const safety = p.locator('.s-aircraft__safety-list');
await safety.scrollIntoViewIfNeeded();
await p.waitForTimeout(200);
await safety.screenshot({ path: join(OUT, 'crop-safety.png') });

const modes = p.locator('.s-aircraft__mode-grid');
await modes.scrollIntoViewIfNeeded();
await p.waitForTimeout(200);
await modes.screenshot({ path: join(OUT, 'crop-modes.png') });

const specs = p.locator('.s-aircraft__spec-grid');
await specs.scrollIntoViewIfNeeded();
await p.waitForTimeout(200);
await specs.screenshot({ path: join(OUT, 'crop-specs.png') });
